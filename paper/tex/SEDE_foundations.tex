...
\title{Is the volume-law horizon a postulate or a consequence? A
reduction to a driven non-equilibrium conjecture for Structural Entropy
Dark Energy}
...
The entropic drive is a variational statement \emph{within} the assumed
Barrow family ---
\(dF/d\Delta  = -T\cdot S_\Delta \cdot\)½·\(ln(A/A_{0}) < 0\) at every
\(\Delta\) for any horizon above Planck area --- and is
near-tautological there, so the load is carried not by the tilt but by
two independent inputs: the capacity ceiling \(\Delta  \leq  1\), proved
in the companion count paper with no thermodynamic input, and gravity's
cooperativity, which supplies the bistability that makes the outcome a
\emph{locked branch} rather than a tunable value. What remains genuinely
conditional is that the two-well landscape presupposes an accessible
volume phase, the mean-field treatment is extrapolated to the N ∼ O(1)
quantum-gravity regime where its tools are not controlled, and the one
concrete model of de Sitter holography (double-scaled SYK) currently
maps to the \emph{area} branch --- so the mechanism relocates and
narrows the postulate, it does not eliminate it. Its lasting value is to
convert an untestable counting postulate into a falsifiable one: a
pre-registered point prediction \(\Delta  = 1\), tested to a forecast
\(\sigma (\Delta ) \approx  0.09\) by DESI DR3 + Euclid. On public data
the model-level statement is modest and is the one we lead with: at
equal parameter count the zero-parameter \(\Delta  = 1\) model is
favoured over \(\Lambda \mathrm{CDM}\) at the cosmology paper's
\(\Delta \mathrm{DIC} \approx  -3\) --- a mild-to-moderate preference,
not a detection. Underneath it, a \emph{profile}-likelihood diagnostic
(the marginalised posterior awaits DR3 + Euclid) shows the
structure-gated family sitting at the volume endpoint,
\(\Delta  = 0.93\) {[}0.83, 1.02{]}, which the un-gated Barrow family
pays \(\Delta \chi ^{2} \approx  2400\) to reach; profile statements of
this kind overstate the marginalised signal by ∼two orders of magnitude
in \(\chi ^{2}\) and are reported as diagnostics, not exclusions. The
cosmological results of the cosmology paper stand independently of the
reduction attempted here.
\end{abstract}
...
Figure 1 sets out the reduction in outline; the sections that follow
take each tier in turn.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../results/foundations_fig1_reduction.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../results/foundations_fig5_inferences.png}}
...
\textbf{The shape of the argument, stated first.} Because a selection
argument for \(\Delta  = 1\) that used \(\Delta  = 1's\) own free energy
would be circular, we set out the logical structure before the details,
and note which input carries which part of the load.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{A drive}, from thermodynamics. Within the Barrow family,
  \(F(\Delta ) = E - T_\mathrm{AH}\) \(S_\Delta\) satisfies
  \(dF/d\Delta  = -T_\mathrm{AH}\)
  \(S_\Delta \cdot\)½·\(ln(A/A_{0}) < 0\) at \emph{every} \(\Delta\),
  for any horizon larger than a Planck area. This is a derivative
  \emph{across} the family, not an evaluation at its endpoint --- but it
  is also close to tautological, since \(S_\Delta\) is constructed to
  increase with \(\Delta\) when \(A > A_{0}\). The drive establishes a
  direction and nothing more.
\item
  \textbf{A ceiling}, from counting. \(\Delta  \leq  1\) is a theorem of
  the companion count paper \cite{Pandev:2026count}, proved from a
  finite per-site leg budget with no thermodynamic input and no
  reference to this argument. It is the ceiling, not the drive, that
  supplies the \emph{value}.
\item
  \textbf{Discreteness}, from gravity. Cooperativity
  \(J = \lambda _{\mathrm{max}}(W_{\mathrm{grav}}) \gg  J_{\mathrm{c}}\)
  makes the landscape bistable with a barrier ∼\(N\cdot T_\mathrm{AH}\),
  so the outcome is a \emph{locked branch} --- area or volume --- rather
  than a continuously tunable exponent. This is why no intermediate
  \(\Delta\) is available to be fitted.
\item
  \textbf{A branch, from history.} Which of the two locked branches a
  given horizon occupies is set at its birth and held by long-range
  hysteresis: continuous growth through the ordering bifurcation selects
  volume; abrupt formation traps the area branch. This is what makes the
  black hole's \(\Delta _\mathrm{BH} = 0\) a prediction rather than an
  accommodation.
\end{enumerate}

No step assumes its conclusion: the direction comes from thermodynamics,
the value from an independently-proved combinatorial ceiling, the
discreteness from gravitational cooperativity, and the branch from
formation history. The load-bearing \emph{physical} claim is (4) ---
that formation history selects the branch --- and that is where this
argument should be attacked, not at the tilt.
...
\item
  ...
  energy, is \emph{monotonic} in \(\Delta\): differentiating across the
  family at fixed horizon data,
  \begin{equation}\frac{dF}{d\Delta} \;=\; -\,T_{\rm AH}\,S_\Delta\,\tfrac{1}{2}\ln\!\frac{A}{A_0} \;<\; 0
  \qquad\text{for every }\Delta,\ \text{whenever } A > A_0,\end{equation} so F(0) =
  0 by the first law and F decreases to \(\Delta  = 1\) with no barrier
  and no second minimum. Two readings must be kept apart. The derivative
  is evaluated at \emph{every} member of the family, so the statement is
  a property of \(F(\cdot )\) over \(\Delta  \in  [0\),1{]} and not an
  evaluation of F at the endpoint --- the drive does not presuppose the
  branch it favours. But neither is it deep: \(S_\Delta\) is constructed
  to increase with \(\Delta\) for \(A > A_{0}\), so ``the free energy
  prefers larger \(\Delta\)'' is close to a restatement of that
  construction. The drive therefore fixes a \emph{direction} only; the
  \emph{value} \(\Delta  = 1\) comes from the independently-proved
  ceiling of the count paper (preamble, item 2), and the fact that the
  outcome is a discrete branch rather than a tunable exponent comes from
  gravity (item 3). Bistability is therefore not a consequence of Barrow
  entropy: Barrow supplies the entropic drive toward \(\Delta  = 1\),
  not the two-phase structure. (This is the actual content of the GSL
  argument of §\ref{sec-8-1} --- it drives \(\Delta\) → 1, it does not
  by itself produce a spinodal.)
\item
...
Figure 3 shows the resulting two-phase landscape and the coupling that
makes it bistable.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../results/criticality_soc.png}}
...
hydrodynamics, result iv, is a separate matter). \emph{The landscape
variable here is the order parameter m of (ii) --- the fraction of
horizon bonds on the bulk side of the min-cut --- and not the
cosmological gate \(f_{\mathrm{sat}}\); the two are distinct coordinates
and must not be read against each other (see the caution
below).}}\label{fig:3}
\end{figure}

\textbf{The order parameter is not the gate.} Because panel \emph{(a)}
is labelled by the bare gate, the landscape invites a reading in which
\(f_{\mathrm{sat}}\) is the order parameter of the double well. It is
not, and the distinction matters for what §\ref{sec-5} does and does not
claim. The order parameter m is a \emph{free-energy} coordinate: it
relaxes by gradient flow, \(\partial _\tau\) \(m = -\Gamma\) F′(m), a
branch-\emph{selecting} dynamics fixed by the shape of F and driven by
the pitchfork at the horizon's birth. The gate \(f_{\mathrm{sat}}\) is a
\emph{cumulative} coordinate obeying activation kinetics
\(\partial _{\mathrm{x}}\) \(f = \gamma (1 - f)\), whose rate vanishes
as f → 1 --- a monotone filling that can never overshoot and that
crosses no barrier. A variable whose own equation of motion switches off
at the endpoint it approaches cannot be the order parameter of a
bistable potential. The division of labour is therefore clean, and is
what §\ref{sec-5} asserts: the \emph{branch} --- which well the horizon
sits in --- is fixed once, at birth, by m (result iii);
\(f_{\mathrm{sat}}\) is the subsequent \emph{occupancy} of the
already-selected volume capacity, not a second branch selector. Nothing
in the selection argument depends on the gate, and no critical behaviour
in \(f_{\mathrm{sat}}\) is claimed (§\ref{sec-6}).
...
Figure 4 shows the selection at the ordering bifurcation described
above.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{../results/foundations_fig_birth.png}}
...
Level 3 already has a present-data instalment, and we state the
model-level result first because it is the one that carries evidential
weight. On DESI DR2 BAO + Pantheon+ + CMB distance priors + cosmic
chronometers \(+ f\sigma 8\), in one CAMB-in-the-loop pipeline, SEDE at
\(\Delta  = 1\) --- carrying \emph{zero} dark-sector parameters --- is
favoured over \(\Lambda \mathrm{CDM}\) at
\textbf{\(\Delta \mathrm{DIC} \approx  -3\)} (the cosmology paper's
headline \cite{Pandev:2026cosmology}), fits 2.9 \(\chi ^{2}\)-units
better at equal parameter count, and sits within 1.4 AIC-units of
w0waCDM, which spends two extra parameters. That is a mild-to-moderate
preference, not a detection, and it is the honest summary of what
present data say about the model.

Two \emph{diagnostic} profile-likelihood statements sit underneath it,
and must not be read as posterior measurements. First, the un-gated
Hubble-cutoff Barrow family pays \(\Delta \chi ^{2} = +2416\) to sit at
\(\Delta  = 1\), reproducing in this related family the community result
that bare Barrow HDE cannot live at the volume endpoint
\cite{Luciano:2025elo}, whereas the gated family pays only
\(\Delta \chi ^{2} = +0.8\) --- the quantitative sense in which the
structure gate is what makes a volume-law count observationally viable
at all. Second, the gated family's profile interval is
\(\Delta  = 0.93\), 68\% CL {[}0.83, 1.02{]}, with all base parameters
re-optimised at each \(\Delta\), and the pre-registered window lies
inside it. Both are \emph{profile} statements: \(\Delta  = 0\) is
disfavoured at \(\Delta \chi ^{2} \approx  371\) along that profile,
which overstates the marginalised model-level signal
\((\Delta \mathrm{DIC} \approx  -3)\) by roughly two orders of magnitude
in \(\chi ^{2}\) and must not be quoted as a posterior exclusion. The
full marginalised posterior remains the DR3 + Euclid target, and the
cosmology paper (§5.6) reserves the label ``marginalised measurement''
for it.
...
\href{https://doi.org/10.5281/zenodo.21525520}{10.5281/zenodo.21525520}.
...
Claude Opus 4.x (Anthropic) --- assisted with drafting and editing the
